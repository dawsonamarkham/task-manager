...
import './AuthForm.css';
import { createAcount } from '../api/auth';
...
// Account Creation Form, validates proper email and password with password confirmation
class AccCreateForm extends Component<AccCreateProps, AccCreateState> {
    ...
    // Identify event source and update state/errors accordingly
    ...
    // Used to disable if validation has failed
    allowLogin(): boolean {
        return !(
            this.state.email !== '' &&
    ...
    // If successful, go to Task View. Otherwise, display error.
    handleResp(tkn?: string, err?: any): void {
        if (tkn) {
            this.props.handleAuthUpdate(tkn);
        }
        else if (err && axios.isAxiosError(err)) {
